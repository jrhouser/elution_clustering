...
def main():
	...
	#df=pd.read_table('./testdata/PA/GMM_bayes_0.9_cluster.p',names=['C'])
	#cluster_prediction_out=[]
	#for c in df['C']:
	#	cluster_prediction_out.append(c.split(','))

	cluster_prediction_out=cPickle.load(open('./testdata/PA/GMM_cluster_bayes_compare_ms1_PA_dof5_bayesGMM_clusters.p','rb'))

	#print cluster_prediction_out
	metric,cluster_prediction_out = cluster_gmm_core(X,clustN,args.base_output_filename,standard_complexes,args.standard_filenames.split(),df_concat,args.cluster_bayes,[cluster_prediction_out])
...
def cluster_gmm_core(X,clustN,outputfilename,standard_complexes,standard_filenames,df_concat,cluster_bayes,cluster_prediction_out):
    #cluster_prediction_out=[]
    metric = pd.DataFrame(index=[str(x) for x in clustN])
    
    # print cluster_prediction_out
    print "Start clustering"
    for c,cluster_prediction in enumerate(cluster_prediction_out):
	...
	try: 
		cluster_prediction.to_csv(outputfilename+ '_clusters.csv')
	except:
		None
	...
	for i,gold_standard_complexes in enumerate(standard_complexes):
		...
		try:
		    ...
		    metric.loc[str(c)+ ' ' +compare_type,'clique_precision_mean_normalize_weighted'] = ccmm_normalize_weighted['precision_mean']
